...
//TvSeriesPagingSource merupakan class paging source untuk menghandle pagination list
class TvSeriesPagingSource(
    private val moviesRepo: MoviesRepo
) : PagingSource<Int, TvSeriesResponse.Movie>(){
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, TvSeriesResponse.Movie> {
        return try {
            val currentLoadingPageKey  = params.key ?: 1 //variable untuk penyimpan halaman terakhir yang di load
            val response = moviesRepo.getPopularTvSeries(page = currentLoadingPageKey) //perintah untuk mengambil data movie dari repo kemudian mendapatkan response
            val responseData = mutableListOf<TvSeriesResponse.Movie>() //variable untuk menyimpan data response
    ...
    override fun getRefreshKey(state: PagingState<Int, TvSeriesResponse.Movie>): Int? {
        return null
    }
